...
def distill(
    *,
    ...
    if (
        source_metadata.get("protocol") != "article1-v2"
        or source_metadata.get("dataset") != dataset
        or int(source_metadata.get("seed", -1)) != seed
    ):
        raise ValueError(
            "teacher cache metadata is incompatible with this distillation condition"
        )
    with np.load(cache, allow_pickle=False) as data:
        logits, labels, indices, mask = (
            data["logits"],
            data["labels"],
            data["proxy_idx"],
            data["M"],
    ...
    proxy_hash = array_sha256(indices)
    ...
    for epoch in range(epochs):
        ...
        for x, positions in loader:
            loss = kd_loss(
                model(x.to(dev)), q.index_select(0, positions.to(dev)), temperature
            )
    ...
    row = {
        ...
        "batch_order_sha256": order.digest.hexdigest(),
        "student_final_sha256": _hash_state(model.state_dict()),
        "updates": epochs * len(loader),
        "student_test_accuracy": accuracy,
        "student_test_nll": nll,
        "cache_creation_commit": artifact_commit(source_metadata),
