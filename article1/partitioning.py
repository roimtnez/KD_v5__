...
def _split_client(
    indices: np.ndarray, seed: int, holdout_fraction: float, test_fraction: float
) -> dict[str, np.ndarray]:
    ...
    return {
        "train_idx": values[n_holdout + n_test :],
        "holdout_idx": values[:n_holdout],
        "test_idx": values[n_holdout : n_holdout + n_test],
    }


def validate_splits(
    proxy_idx: np.ndarray, clients: list[dict[str, np.ndarray]]
) -> None:
    ...
    for client in clients:
        ...
        if any(
            np.intersect1d(roles[i], roles[j]).size
            for i in range(3)
...
def make_partitions(
    ...
    *,
    ...
    rows = [
        _split_client(values, seed + cid + 1, holdout_fraction, test_fraction)
