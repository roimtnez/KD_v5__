...
def kd_config(epochs: int = 30, batch_size: int = 256) -> dict:
    """One shared recipe for execution and exact run-identity reuse."""
    return {
        "epochs": epochs,
...
def _validate(
    logits: np.ndarray, labels: np.ndarray, mask: np.ndarray | None
...
def _routing(
    method: str, z: np.ndarray, y: np.ndarray, m: np.ndarray | None
    ...
    if method == "energy_logit":
        ...
        return (
            _teacher_softmax(score),
            all_selected,
            "softmax_teachers(logsumexp(logits))",
        )
    ...
    *,
    ...
    if method.endswith("_logit"):
        ...
        aggregation = (
            "mean_probabilities"
            if method != "expert_prob_sr"
            else "mean_support_restricted_probabilities"
        )
    ...
    if method in {"expert_prob", "expert_prob_sr"}:
        ...
        metrics["pre_restriction_outside_support_mass"] = (
            float(outside_mass[event].mean()) if event.any() else None
        )
    return Target(
        q.astype(np.float32), weights.astype(np.float32), selected, fallback, metrics
    )
...
def kd_loss(student_logits, q, temperature: float):
    ...
    return (
        F.kl_div(
            F.log_softmax(student_logits / temperature, dim=1), q, reduction="batchmean"
        )
        * temperature**2
    )
